    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <h1 class="text-3xl font-bold text-gray-900 flex items-center gap-2">
          ...
          {{ t('readingList.myTitle') }}
        </h1>
        <div class="relative max-w-md w-full md:w-auto">
          <input type="text" v-model="filterText" :placeholder="t('readingList.filterArticles')"
        ...
        <div v-for="item in currentFilteredList" :key="item.id"
          ...
          <div class="flex-1">
            ...
            <div class="flex items-center gap-3 text-sm">
              ...
              <span class="px-2 py-0.5 rounded-full text-xs font-semibold"
                ...
                {{ item.read ? t('readingList.read') : t('readingList.unread') }}
              </span>
            </div>
          </div>
          ...
          <div class="flex items-center gap-2 self-start sm:self-center">
            <button @click="toggleReadStatus(item.id)"
              ...
              {{ item.read ? t('readingList.markUnread') : t('readingList.markRead') }}
            </button>
            <button @click="removeArticle(item.id)"
              class="p-2 text-red-500 hover:bg-red-50 rounded-md transition-colors" :title="t('common.remove')">
              <DeleteOutlined />
            </button>
          </div>
        </div>
      ...
      <div v-else class="text-center py-20 bg-white rounded-xl shadow-sm border border-gray-100">
        ...
        <h3 class="text-xl font-medium text-gray-500">{{ t('readingList.emptyListTitle') }}</h3>
        <p class="text-gray-400 mt-1">{{ t('readingList.emptyHint') }}</p>
      </div>

...
async function loadIssue() {
  ...
  try {
    ...
    error.value = t('sermonIssue.loadFailed');
  } finally {
    loading.value = false;
  }
}
      ...
      <template v-else>
        <header class="issue-intro mx-auto mb-6 max-w-2xl text-center sm:mb-8">
          ...
          <h1 class="issue-title">
            {{ issue?.title || t('sermonIssue.title') }}
          </h1>
        ...
        <MessengerFlipbookViewer
          ...
          :title="issue?.title || t('sermonIssue.title')"
        />

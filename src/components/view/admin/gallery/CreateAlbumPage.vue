<template>
  <div class="bg-white p-6 rounded-lg shadow-sm border border-gray-200 max-w-4xl mx-auto">
    ...
    <!-- Select photos modal (WordPress-style) -->
    <a-modal
    ...
    <!-- Cover selection modal (pick from any existing photo) -->
    <a-modal
...
// Selection inside the modal (pending until "Add to album")
...
/** Load photos from GET /photos/?skip=0&limit=100 → [{ key, url, uploaded_at, filename }, ...] */
...
async function onFileSelectInModal(e) {
  ...
  for (const file of Array.from(files)) {
    ...
    try {
      // Server returns { key: "af122020782d49cc981746c45beb40f0.jpg" } — use key to get image URL

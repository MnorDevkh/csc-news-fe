<template>
  <div class="bg-white p-6 rounded-md shadow-sm border border-gray-200">
    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
      <div>
        <h2 class="text-2xl font-bold text-gray-800">The Sermon</h2>
        <p class="text-gray-500 text-sm">Manage PDF issues for The Sermon magazine</p>
      </div>
      <button
        @click="goToCreate"
...
async function loadIssues() {
  loading.value = true;
  try {
    const data = await SermonIssueService.getIssues({ skip: 0, limit: 100 });
    issues.value = data.items || [];
  } catch (error) {
    console.error('Failed to load sermon issues', error);
    alert('Failed to load sermon issues');
  } finally {
    loading.value = false;
  }
}
...
function goToCreate() {
  router.push({ name: 'createSermonIssue' });
}

function editIssue(id) {
  router.push({ name: 'editSermonIssue', params: { id } });
}

async function deleteIssueItem(id) {
  if (!confirm('Are you sure you want to delete this issue?')) return;
  try {
    await SermonIssueService.deleteIssue(id);
    await loadIssues();
  } catch (error) {

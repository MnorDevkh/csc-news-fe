  ...
  <div class="bg-white p-8 rounded-md shadow-sm border border-gray-200 max-w-3xl mx-auto">
    <div class="flex items-center justify-between mb-8">
      <h2 class="text-2xl font-bold text-gray-800">
        {{ isEditMode ? 'Edit Sermon Issue' : 'Create Sermon Issue' }}
      </h2>
      <button @click="$router.push({ name: 'adminSermonIssues' })" class="text-gray-500 hover:text-gray-700">
        Cancel
      </button>
    </div>
    ...
    <div v-else-if="loadError" class="text-center py-12">
      ...
      <button
        @click="$router.push({ name: 'adminSermonIssues' })"
...
async function handlePdfSelect(event) {
  ...
  try {
    const { url } = await SermonIssueService.uploadPdf(file);
    form.pdf_url = url;
  } catch (err) {
  ...
  try {
    if (isEditMode.value) {
      ...
      const issue = isUuid(param)
        ? await SermonIssueService.getIssueById(param, { public_only: false })
        : await SermonIssueService.getIssueBySlug(param, { public_only: false });
    ...
    console.error('Error loading sermon issue:', err);
...
async function handleSubmit() {
  ...
  try {
    ...
    if (isEditMode.value) {
      await SermonIssueService.updateIssue(resolvedIssueId.value || route.params.id, payload);
      feedbackType.value = 'success';
      feedbackMsg.value = 'Issue updated successfully!';
    } else {
      await SermonIssueService.createIssue(payload);
      feedbackType.value = 'success';
      feedbackMsg.value = 'Issue created successfully!';
    }
    setTimeout(() => router.push({ name: 'adminSermonIssues' }), 800);
  } catch (err) {
    console.error('Error saving sermon issue:', err);
    const detail = err.response?.data?.detail;
    feedbackType.value = 'error';
    feedbackMsg.value =

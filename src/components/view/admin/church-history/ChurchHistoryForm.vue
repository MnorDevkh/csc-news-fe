<template>
  <div class="bg-white p-6 rounded-md shadow-sm border border-gray-200 max-w-4xl mx-auto">
    <div class="flex items-center justify-between mb-6">
      <h2 class="text-2xl font-bold text-gray-800">Church History</h2>
...
import { ChurchHistoryService } from '@/services/ChurchHistoryService';
...
onMounted(async () => {
  try {
    ...
    console.error('Error loading church history:', error);
    contentBlocks.value = [{ id: nextId(), type: 'text', html: '' }];
  }
});
...
const handleSubmit = async () => {
  try {
    ...
    const payload = {
      ...
      content: JSON.stringify(serializeBlocks()),
    };
    await ChurchHistoryService.update(payload);
    alert('Church history saved successfully!');
  } catch (error) {
    console.error('Error saving church history:', error);

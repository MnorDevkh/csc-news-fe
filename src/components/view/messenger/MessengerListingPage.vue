...
async function loadIssues() {
  ...
  error.value = '';
  try {
    const data = await MessengerService.getIssues({
      skip: 0,
      limit: fetchLimit(),
      is_published: true,
    });
    ...
    error.value = t('messenger.loadListFailed');
  } finally {
    loading.value = false;
  }
}
  ...
  <div :class="embed ? 'w-full' : 'min-h-screen bg-surface py-6 sm:py-10'">
    <div :class="embed ? 'w-full' : 'max-w-6xl mx-auto px-3 sm:px-5'">
      <header v-if="!embed" class="mb-6 sm:mb-8">
        <h1 class="text-3xl font-bold text-stone-900 mb-2 tracking-tight">{{ t('messenger.title') }}</h1>
      ...
      >
        <p class="text-stone-500 text-sm mb-0">{{ t('messenger.noIssues') }}</p>
      </div>

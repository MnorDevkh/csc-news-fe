    ...
    <div class="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
      <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100 hover:shadow-md transition-shadow group">
        <div class="flex items-center justify-between mb-3">
          <div class="h-11 w-11 rounded-md bg-[#4165d1]/8 flex items-center justify-center group-hover:bg-[#4165d1]/12 transition-colors">
            <FileTextOutlined class="text-lg text-[#4165d1]" />
          </div>
      ...
      <div class="rounded-2xl bg-white p-5 shadow-sm ring-1 ring-gray-100 hover:shadow-md transition-shadow group">
        <div class="flex items-center justify-between mb-3">
          <div class="h-11 w-11 rounded-md bg-[#e02838]/10 flex items-center justify-center group-hover:bg-[#e02838]/15 transition-colors">
            <TagsOutlined class="text-lg text-[#e02838]" />
          </div>
          <span class="text-xs font-medium text-[#e02838] bg-[#e02838]/10 px-2 py-1 rounded-md">Active</span>
        </div>
    ...
    <div class="grid grid-cols-1 gap-6 lg:grid-cols-3 pt-4">
      ...
      <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100 lg:col-span-2">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <router-link v-for="action in quickActions" :key="action.name" :to="action.to"
            class="flex flex-col items-center gap-3 p-4 rounded-2xl bg-gray-50/80 hover:bg-[#4165d1]/5 border border-transparent hover:border-[#4165d1]/10 transition-all duration-200 group cursor-pointer">
            ...
            <span class="text-xs font-medium text-gray-600 group-hover:text-[#4165d1] text-center transition-colors">{{ action.name }}</span>
          </router-link>
        </div>
      </div>
      ...
      <div class="rounded-2xl bg-white p-6 shadow-sm ring-1 ring-gray-100">
        ...
        <div class="space-y-4">
          ...
          <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80">
            <div class="h-8 w-8 rounded-md bg-[#4165d1]/10 flex items-center justify-center">
              <GlobalOutlined class="text-[#4165d1] text-sm" />
            </div>
            <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-3 p-3 rounded-xl bg-gray-50/80">
            <div class="h-8 w-8 rounded-md bg-[#e02838]/10 flex items-center justify-center">
              <ClockCircleOutlined class="text-[#e02838] text-sm" />
            </div>
            <div class="flex-1 min-w-0">
...
const quickActions = [
  { name: 'New Article', to: { name: 'createNews' }, icon: FileTextOutlined, iconBg: 'bg-[#4165d1]/8', iconColor: 'text-[#4165d1]' },
  { name: 'Categories', to: { name: 'adminCategories' }, icon: TagsOutlined, iconBg: 'bg-[#e02838]/10', iconColor: 'text-[#e02838]' },
